    ...
    while (report.hasNext()) {
        try {
            ...
            if (!mccListCache[listId]) {
                mccListCache[listId] = { name: listName, negatives: [] };
            }
...
function fetchSharedNegativeLists(cache) {
    ...
    while (listReport.hasNext()) {
         ...
         try {
            ...
            // If a list with this ID already exists (from the MCC cache), skip it.
            // This ensures that we don't re-process MCC lists or overwrite them.
            if (!listId || !listName || !keywordText || !keywordMatchType || cache.sharedLists[listId]) {
                 continue;
            }
